...
st.set_page_config(page_title="Wizard Hat Filter", page_icon="🧙", layout="centered")
...
st.markdown('<p class="title-main">🧙 Wizard Hat</p>', unsafe_allow_html=True)
st.markdown('<p class="title-sub">Real-time AR Face Filter</p>', unsafe_allow_html=True)

HAT_PATH = "assets/wizard-hat.png"

@st.cache_resource
def load_hat():
    img = cv2.imread(HAT_PATH, cv2.IMREAD_UNCHANGED)
    if img is None:
        st.error(f"Could not load asset: {HAT_PATH}")
        st.stop()
    if img.shape[2] == 3:
...
ctx = webrtc_streamer(
    ...
    video_processor_factory=lambda: FaceFilterProcessor(hat_asset),
    ...
    if st.button("📸 Take Picture"):
        ...
        if snapshot is not None:
            ...
            st.download_button(
                label="⬇ Download",
                data=buf,
                file_name="wizard_hat_photo.png",
                mime="image/png",
            )
        else:
